...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased bg-gray-50 text-gray-800 min-h-screen text-[15px]">
        ...
        <div class="flex min-h-screen" x-data="{ sidebarOpen: false }">
            ...
            <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
                   class="fixed inset-y-0 left-0 z-30 w-72 bg-[#1a2035] flex flex-col transition-transform duration-300 lg:translate-x-0 lg:static lg:z-auto shrink-0">

                <!-- Brand -->
                <div class="flex items-center gap-3 px-7 py-6">
                    <div class="h-10 w-10 rounded-xl bg-teal-500 flex items-center justify-center shrink-0">
                        <svg class="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                        </svg>
                    </div>
                    <span class="text-white font-bold text-xl tracking-tight">Donation Module</span>
                </div>

                <!-- Nav -->
                <nav class="flex-1 px-5 pb-5 overflow-y-auto">
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest px-3 mb-3 mt-2">Menu</p>

                    <!-- Dashboard / Portal -->
                    <a href="{{ Auth::user()->hasRole('Donor') ? route('donor.portal') : route('dashboard') }}"
                       class="flex items-center gap-3.5 px-3.5 py-3 rounded-lg text-[15px] font-medium mb-1 transition-all
                    ...
                    <a href="{{ route('donor.portal') }}"
                       class="flex items-center gap-3.5 px-3.5 py-3 rounded-lg text-[15px] font-medium mb-1 transition-all text-slate-400 hover:text-white hover:bg-white/5">
                        <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div x-data="{ reportsOpen: {{ request()->routeIs('reports.*') ? 'true' : 'false' }} }">
                        <button @click="reportsOpen = !reportsOpen"
                                class="w-full flex items-center gap-3.5 px-3.5 py-3 rounded-lg text-[15px] font-medium mb-1 transition-all
                    ...
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest px-3 mb-3">Account</p>
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                                class="w-full flex items-center gap-3.5 px-3.5 py-3 rounded-lg text-[15px] font-medium transition-all text-slate-400 hover:text-red-400 hover:bg-red-500/10">
                            <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
